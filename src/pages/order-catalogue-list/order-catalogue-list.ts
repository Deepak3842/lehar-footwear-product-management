...
export class OrderCatalogueListPage {
  ...
  thalaFavourites = {
    celebrityImage: 'assets/imgs/dhoni.jpg', // MS Dhoni image
    celebrityName: 'LEHAR',
    title: 'FAVOURITES',
  };
  ...
  getStockInwordTodayList() {
    this.service.addData({ search: this.filter.search }, 'AppOrder/stock_inword_today').then((r) => {
      if (r['statusCode'] == 200) {
  ...
  getNewArrvalsList() {
    ...
    this.service.addData({ search: this.filter.search }, 'AppOrder/new_arrivals').then((r) => {
      if (r['statusCode'] == 200) {
  ...
  getHandpickedForYouList() {
    this.service.addData({ search: this.filter.search }, 'AppOrder/handpicked_for_you').then((r) => {
      if (r['statusCode'] == 200) {
  ...
  getThalaFavouritesList() {
    this.service.addData({ search: this.filter.search }, 'AppOrder/thala_favourites').then((r) => {
      if (r['statusCode'] == 200) {
  ...
  getSpecialCollectionList() {
    this.service.addData({ search: this.filter.search }, 'AppOrder/special_collection').then((r) => {
      if (r['statusCode'] == 200) {
