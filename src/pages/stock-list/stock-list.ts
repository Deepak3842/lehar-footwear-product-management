...
export class StockListPage {
  ...
  loadCartQuantity() {
    try {
      ...
      if (savedCart) {
        ...
        this.totalCartQuantity = cartItems.reduce((sum, item) => sum + item.quantity, 0);
      } else {
  ...
  getProductList() {
    this.service
      .addData(
        { cat_name: this.CategoryName, cat_id: this.selectedCategory },
      ...
      .then((resp) => {
        if (resp['statusCode'] === 200) {
          ...
          const map = new Map();
          [...this.stockList, ...this.categoryProductList].forEach(item => {
            map.set(item.id, item);
          });
          this.finalDisplayList = Array.from(map.values());
        } else {
          this.service.errorToast(resp['statusMsg']);
        }
      });
  ...
  getStockList(type: any) {
    this.service.addData({ 'type': type, 'gender': this.gender, 'sole_type': this.soleType }, 'AppOrder/getProductWithFilters').then((r) => {
      if (r['statusCode'] === 200) {
