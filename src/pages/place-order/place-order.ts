...
export class PlaceOrderPage {
  ...
  getMainStockList() {
    this.service.addData({ 'product_id': this.data.id }, 'AppOrder/stockList').then((r) => {
      if (r['statusCode'] === 200) {
  ...
  onQuantityChange(sizeItem: any, event: any, variant: any) {
    ...
    // Check if popup should be shown (only if quantity increased from 0)
  ...
  addToCart() {
    ...
    this.colorVariants.forEach(variant => {
      ...
      selectedSizes.forEach(sizeItem => {
        ...
        if (existingIndex > -1) {
          this.cartItems[existingIndex].quantity += sizeItem.quantity;
          ...
          this.cartItems.push({
            ...
            total_amount: itemPrice * sizeItem.quantity
          });
        }
        addedCount++;
      });
